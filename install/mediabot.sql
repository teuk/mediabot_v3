...
-- ---------------------------------------------------------------------------
-- ACHIEVEMENT_PROFILE / IDENTITY / UNLOCK / PROGRESS (mb646)
-- Durable achievements storage. One logical profile is scoped to one channel;
-- IRC nick/user@host variants are aliases attached to that profile.
-- ---------------------------------------------------------------------------
CREATE TABLE `ACHIEVEMENT_PROFILE` (
...
CREATE TABLE `ACHIEVEMENT_IDENTITY` (
...
ALTER TABLE `RSS_FEED`
  ADD CONSTRAINT `fk_rss_feed_channel` FOREIGN KEY (`id_channel`) REFERENCES `CHANNEL` (`id_channel`) ON DELETE CASCADE ON UPDATE CASCADE,
  ADD CONSTRAINT `fk_rss_feed_created_by` FOREIGN KEY (`created_by`) REFERENCES `USER` (`id_user`) ON DELETE SET NULL ON UPDATE CASCADE;

ALTER TABLE `RSS_ITEM`
  ADD CONSTRAINT `fk_rss_item_feed` FOREIGN KEY (`id_rss_feed`) REFERENCES `RSS_FEED` (`id_rss_feed`) ON DELETE CASCADE ON UPDATE CASCADE;
...
ALTER TABLE `QUOTES`
  ...
  ADD CONSTRAINT `fk_quotes_user`    FOREIGN KEY (`id_user`)    REFERENCES `USER`    (`id_user`)    ON DELETE SET NULL ON UPDATE CASCADE;
