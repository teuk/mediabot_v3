...
use File::Spec;
sub _slurp_236 { open my $fh,'<:encoding(UTF-8)',$_[0] or die $!; local $/; <$fh> }
sub _sub_236 { my($s,$n)=@_; my $re=qr/^[ \t]*sub[ \t]+\Q$n\E\b[^{]*\{/m;
...
return sub {
    ...
    # mbKarmaHist_ctx
    my $kh = _sub_236($src, 'mbKarmaHist_ctx');
    ...
    # Dispatch
    my $mm = _slurp_236(File::Spec->catfile('.', 'Mediabot', 'Mediabot.pm'));
