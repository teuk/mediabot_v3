...
return sub {
    ...
    my $overview = format_rss_feed_overview('#test',
    ...
    $assert->like($overview->[0], qr/Flux RSS #test.*?\[Korben\].*?5 min\/max3.*?25 items.*?OK/s, 'mb694-905: overview carries operational status');
    ...
    my $lines = format_rss_feed_info_lines({
        label=>'Korben',channel=>'#test',url=>'https://korben.info/feed',enabled=>1,
    ...
    $assert->is($lines->[0], 'RSS [Korben] on #test — ON · OK', 'mb694-905: info headline exposes ON + health');
    ...
    my $err = format_rss_feed_info_lines({
        label=>'Broken',channel=>'#test',url=>'https://example.org/feed',enabled=>1,
    ...
    $assert->is($err->[0], 'RSS [Broken] on #test — ON · ERROR', 'mb694-905: error outranks pending');
