...
return sub {
    ...
    $assert->is($contract->{milestone}, 'MB761',
        'machine contract records the write-command adoption milestone');
    ...
    $assert->is(join(',',
        ...
        'learn,forget',
        'machine contract names exactly the two write migrations');
    $assert->is(join(',',
        @{ $contract->{factoid_write_migration}{excluded_commands} }),
        'whatis,?keyword',
        'recall-mutating commands remain excluded');
    ...
    $assert->is($manifest->{version}, '1.1.0',
        'factoid package version advances for write adoption');
    $assert->is(join(',', @{ $manifest->{capabilities} }),
        'data.factoids.read,data.factoids.write,irc.notice',
        'package requests only factoid read/write and notice capabilities');
    for my $command (qw(factoid factoids learn forget)) {
    ...
    $assert->ok(!exists($manifest->{commands}{whatis}),
        'package cannot mount recall-counter mutation');
    ...
    $assert->like($guide,
        qr/`observe`.*suppresses the v3 write before the service/s,
        'pilot guide freezes non-duplicating shadow writes');
    $assert->like($guide,
        qr/`whatis` and `\?keyword` remain historical/,
        'pilot guide preserves the recall boundary');
    $assert->like($guide,
