...
return sub {
    ...
    $assert->is($decision->{kind}, 'aside',
        'mb739: first compact-context candidate is an autonomous Aside');
    ...
    $assert->like($log, qr/^\[SPARK_DRYRUN\].*kind=aside/,
        'mb739: Aside candidate has a grep-friendly metadata log');
    ...
    $assert->ok($next->{kind} ne 'aside',
        'mb739: autonomous schedule avoids immediate event repetition');
