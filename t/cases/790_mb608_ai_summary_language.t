# t/cases/790_mb608_ai_summary_language.t
# =============================================================================
...
return sub {
    ...
    $assert->is(join(',', sort keys %$S), 'en,es,fr',
        'mb608-790: trois langues declarees');
    ...
    $assert->is($complete, 3, 'mb608-790: chaque langue a toutes les cles');
    $assert->is(Mediabot::External::Claude::_summary_lang_strings('de')->{name},
        'English', 'mb608-790: langue inconnue -> repli anglais');
    $assert->is(Mediabot::External::Claude::_summary_lang_strings(undef)->{name},
        'English', 'mb608-790: langue absente -> repli anglais');
    $assert->is($S->{fr}{name}, 'French', 'mb608-790: fr demande du francais au modele');
    $assert->is($S->{es}{name}, 'Spanish', 'mb608-790: es demande de l espagnol');

    # [2] libelles de periode
    my $L = \&Mediabot::External::Claude::_summary_period_label;
    $assert->is($L->('fr', 'today'), " (aujourd'hui)",
        'mb608-790: today en francais');
    $assert->is($L->('es', 'yesterday'), ' (ayer)', 'mb608-790: yesterday en espagnol');
    $assert->is($L->('en', 'today'), ' (today)', 'mb608-790: anglais inchange');
    $assert->is($L->('fr', 'days', 7), ' (7 derniers jours)',
        'mb608-790: periode a parametre en francais');
    $assert->is($L->('fr', 'last', 125), ' (depuis 2h05m)',
        'mb608-790: last au format heures/minutes');
    $assert->is($L->('fr', 'last', 12), ' (depuis 12m)',
        'mb608-790: last au format minutes');
    $assert->is($L->('fr', ''), '', 'mb608-790: aucun filtre = aucun libelle');
    $assert->is($L->('fr', 'inconnu'), '', 'mb608-790: cle inconnue = rien');
    ...
    $assert->ok(defined $grep_block && $grep_block =~ /forced_lang/,
        'mb608-790: le jeton de langue est extrait dans la passe grep');
    my $grep_pos = index($src, '@args = grep {');
    my $nick_pos = index($src, 'my $filter_nick =');
    $assert->ok($grep_pos > 0 && $nick_pos > $grep_pos,
        'mb608-790: ... donc AVANT le parsing du filtre nick');
    $assert->like($grep_block, qr/lang\[=:\]\(\[a-z\]\{2\}\)/,
        'mb608-790: la forme lang=xx est acceptee');
    $assert->like($grep_block, qr/\\A\(\?:en\|fr\|es\)\\z/,
        'mb608-790: le jeton nu en|fr|es est accepte');
    $assert->like($grep_block, qr/nick=\(\.\+\)/,
        'mb608-790: nick=<pseudo> echappe les collisions avec les options');
    ...
    $assert->ok(ref($probe) eq 'CODE', 'mb608-790: sonde du parseur compilee');
    my $r = $probe->(qw(today fr));
    $assert->is(($r->{lang} // ''), 'fr', 'mb608-790: sonde — jeton nu fr capte');
    $assert->is(join(',', @{ $r->{rest} }), 'today',
        'mb608-790: sonde — la periode survit au filtrage');
    $r = $probe->(qw(today teuk));
    $assert->ok(!defined $r->{lang} && join(',', @{ $r->{rest} }) eq 'today,teuk',
        'mb608-790: sonde — un pseudo ordinaire n est PAS pris pour une langue');
    $r = $probe->(qw(7d 3l public lang=es SlaY));
    $assert->ok(($r->{lang} // '') eq 'es' && $r->{public} && $r->{lines} == 3
                && join(',', @{ $r->{rest} }) eq '7d,SlaY',
        'mb608-790: sonde — lang=es coexiste avec public, 3l, periode et nick');
    $r = $probe->(qw(lang=de));
    $assert->ok(!defined $r->{lang} && ($r->{bad} // '') eq 'de',
        'mb608-790: sonde — un code inconnu est signale, pas accepte');
    $r = $probe->(qw(today lang=en nick=fr));
    $assert->ok(($r->{lang} // '') eq 'en' && ($r->{nick} // '') eq 'fr'
                && join(',', @{ $r->{rest} }) eq 'today',
        'mb608-790: sonde — nick=fr reste un filtre nick avec langue explicite');
    $r = $probe->(qw(today fr nick=fr));
    $assert->ok(($r->{lang} // '') eq 'fr' && ($r->{nick} // '') eq 'fr'
                && join(',', @{ $r->{rest} }) eq 'today',
        'mb608-790: sonde — langue fr et pseudo fr peuvent coexister');
    $r = $probe->(qw(10));
    $assert->ok(!defined $r->{lang} && !defined $r->{bad},
        'mb608-790: sonde — sans jeton, rien n est force (defaut = canal)');

    # [4] resolution et repli
    $assert->like($src, qr/my \$lang = \$forced_lang\s*\n\s*\|\|.*channel_lang/s,
        'mb608-790: force > langue du canal');
    $assert->like($src, qr/\$lang = 'en' unless \$lang =~/,
        'mb608-790: ... et repli anglais si la valeur sort du trio');
    $assert->like($src, qr/Unsupported language '\$bad_lang'/,
        'mb608-790: un code inconnu previent l appelant');

    # [5] le prompt garde ses metadonnees anglaises
    $assert->like($src, qr/"Summarise this IRC conversation\$\{who_str\}\$\{date_label\} "/,
        'mb608-790: le prompt conserve son entete anglais');
    $assert->like($src, qr/\$len_str, in \$lang_name:/,
        'mb608-790: seule une instruction de langue est ajoutee');
    $assert->like($src, qr/\$date_label\s+= ' \(today\)';/,
        'mb608-790: le libelle du prompt reste anglais');

    # [6] documentation
    $assert->like($src, qr/Forcer avec en \| fr \| es, ou lang=fr/,
        'mb608-790: l aide de la sous-commande documente la langue');
    $assert->like($src, qr/nick=<pseudo>/,
        'mb608-790: l aide documente l echappement des pseudos homonymes');
    my $mb = do { open my $fh, '<:encoding(UTF-8)', 'Mediabot/Mediabot.pm'
        or die $!; local $/; <$fh> };
    $assert->like($mb, qr/summary \[periode\] \[N\] \[Nl\] \[public\] \[en\|fr\|es\]/,
        'mb608-790: la ligne de commande publique annonce la langue');
};
